...
with io.open('db_generated_datasets/ner_dataset.txt', 'r', encoding='utf-8') as tsvin:
    temp_doc_tokens = []
    tsvin = csv.reader(tsvin, delimiter='\t')
    for row in tsvin:
        if not row:
            total_sents.append(temp_doc_tokens)
            temp_doc_tokens = []
        else: 
            temp_doc_tokens.append((row[0], row[1], row[2])) #append tuples
