...
class TaggerTests(unittest.TestCase):
    ...
    # check if same token strs returned in first slot of tuple 
    def test_pos_tag_sner_labels_not_altered(self):
        tagger = Tagger()
        input, output = self.pos_tag_get_results(tagger)
    ...
    def nonlocal_ner_get_results(self, tagger):
        input =  [[("this", "", "", "d"), ("is", "", "", "d"), ("a", "", "", "d"), ("test", "", "", "d")],
                [("this", "", "", "d"), ("is", "", "", "d"), ("a", "", "", "d"), ("test", "", "", "d")]]
        output = tagger.nonlocal_ner_tag([input])
        return input, output

    def test_nonlocal_ner_nr_lines(self):
        tagger = Tagger()
        input, output = self.nonlocal_ner_get_results(tagger)
        self.assertEqual(len(input), len(output))

    def test_nonlocal_ner_same_nr_tokens(self):
        tagger = Tagger()
        input, output = self.nonlocal_ner_get_results(tagger)
    ...
    # check if all tuples contain 4 slots
    def test_nonlocal_ner_4_slots_each_tuple(self):
        tagger = Tagger()
        input, output = self.nonlocal_ner_get_results(tagger)
    ...
    # check if same token strs returned in first slot of tuple 
    def test_nonlocal_ner_same_token_strs_returned(self):
        tagger = Tagger()
        input, output = self.nonlocal_ner_get_results(tagger)
